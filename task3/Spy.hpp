...
public:
    ...
    void setLogger(Logger &&logger) requires((!std::copyable<T> || std::copyable<std::remove_reference_t<Logger>>)&&(!std::movable<T> || std::movable<std::remove_reference_t<Logger>>))
    {
