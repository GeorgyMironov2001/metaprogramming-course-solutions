...
namespace type_lists {
    ...
    template<class TL> requires(TypeSequence<TL>)
    struct Help_struct_for_to_tuple<TL> {
        using Type = typename Help_struct_for_to_tuple<typename TL::Tail, typename TL::Head>::Type;
    };
    template<class TL, class... U> requires(TypeSequence<TL>)
    struct Help_struct_for_to_tuple<TL, U...> {
    ...
    template<template<class> typename F, typename TL> requires(std::same_as<typename TL::Tail, Nil>)
    struct Map_struct<F, TL> {
        using Head = F<typename TL::Head>;
        using Tail = Nil;
    };
    template<template<class> typename F, typename TL>
    ...
    template<template<class> typename F, typename TL> requires(std::same_as<TL, Nil>)
    struct Map_Nil<F, TL> {
        using Type = Nil;
    };
    template<template<class> typename F, typename TL>
    ...
    template<bool b, template<class> typename P, typename TL>
    struct next_fit_for_P {

    };
    template<template<class> typename P, typename TL>
    struct next_fit_for_P<true, P, TL> {
        using T = TL;
    };
    template<template<class> typename P>
    struct next_fit_for_P<true, P, Nil> {
        using T = Nil;
    };
    template<template<class> typename P, typename TL>
    struct next_fit_for_P<false, P, TL> {
        using T = typename next_fit_for_P<P<typename TL::Tail::Head>::Value, P, typename TL::Tail>::T;
    };
    template<template<class> typename P>
    struct next_fit_for_P<false, P, Nil> {
        using T = Nil;
    };
    template<template<class> typename P, typename TL>
    struct Filter1;
    ...
    template<template<class> typename P, typename TL>
    struct help_for_filter {
        using Type = Filter1<P, TL>;
    };
    template<template<class> typename P>
    struct help_for_filter<P, Nil> {
        using Type = Nil;
    };
    template<template<class> typename P, typename TL>
    struct Filter1 {
        using block = typename next_fit_for_P<P<typename TL::Head>::Value, P, TL>::T;
        using Head = typename block::Head;
        using Tail = typename help_for_filter<P, typename block::Tail>::Type;
    };
    template<template<class> typename P, typename TL>
    using Filter = typename help_for_filter<P, TL>::Type;
    ...
    template<typename L, typename R> requires (std::same_as<typename L::Tail, Nil> ||
                                               std::same_as<typename R::Tail, Nil>)
    struct Zip2_struct<L, R> {
        using Head = TTuple<typename L::Head, typename R::Head>;
        using Tail = Nil;
    };
    ...
    template<typename L, typename R> requires (std::same_as<L, Nil> ||
                                               std::same_as<R, Nil>)
    struct base<L, R> {
        using Type = Nil;
    };
    ...
    template<typename T, typename TL> requires (!std::same_as<T, Nil>)
    struct get_head_cons<Cons<T, TL>> {
    ...
    template<typename T> requires (!std::same_as<T, Nil>)
    struct get_head_cons<Cons<T, Nil>> {
    ...
    template<typename T, typename TL> requires (!std::same_as<typename T::Tail, Nil>)
    struct get_tail_cons<Cons<T, TL>> {
    ...
    template<typename T> requires (!std::same_as<typename T::Tail, Nil>)
    struct get_tail_cons<Cons<T, Nil>> {
        using Type = Cons<typename T::Tail, Nil>;
        static constexpr bool Cons_tail_no_Nil = true;
    };
    template<typename T> requires(std::same_as<typename T::Tail, Nil>)
    struct get_tail_cons<T> {
        using Type = Nil;
        static constexpr bool Cons_tail_no_Nil = false;
    };
    ...
    template<typename ConsTL> requires (!get_tail_cons<ConsTL>::Cons_tail_no_Nil)
    struct zip_struct<ConsTL> {
        using Head = ToTuple<typename get_head_cons<ConsTL>::Type>;
        using Tail = Nil;
    };
    ...
    template<typename... TL> requires (!get_head_cons<FromTuple<TTuple<TL...>>>::no_Nil)
    struct base_zip<TL...> {
        using Type = Nil;
    };
    template<typename... TL>
